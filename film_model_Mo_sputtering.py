...
for i, tk in enumerate(thickness):
    if tk < tk0:
        t_minus = (tk0-tk)/5
        print(f'Травление Mo на глубину {t_minus*1e9}')
        tk = 0
    else:
        tk = tk-tk0
    ...
    Z[mask1] += 1+r1+2*np.sqrt(r1)*np.cos(rho1-k*(L[mask1]+2*dz))
    Z[mask2] += 1+r2+2*np.sqrt(r2)*np.cos(rho2-k*(L[mask2]+2*(dz+tk-t_minus)))
